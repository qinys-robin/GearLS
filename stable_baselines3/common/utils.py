...
def obs_as_tensor(obs: Union[np.ndarray, dict[str, np.ndarray]], device: th.device) -> Union[th.Tensor, TensorDict]:
    ...
    if isinstance(obs, np.ndarray):
        ...
        for key,_obs in obs.items():
            try:
                tensor_dict[key] = th.as_tensor(_obs, device=device)
            except ValueError:
                if isinstance(_obs, Data):
                    tensor_dict[key] = Batch.from_data_list([_obs.to(device)])
                else:
                    tensor_dict[key] = Batch.from_data_list([i.to(device) for i in _obs])
        return tensor_dict
    else:
