...
class BaseBuffer(ABC):
    ...
    def swap_and_flatten(arr: np.ndarray) -> np.ndarray:
        ...
        if isinstance(arr, list):
            batches = []
            for gbatch in arr:
                batches.extend(gbatch.to_data_list())
    ...
    def to_torch(self, array: np.ndarray, copy: bool = True) -> th.Tensor:
        ...
        try:
            ...
            return Batch.from_data_list([i.to(self.device) for i in array])
...
class RolloutBuffer(BaseBuffer):
    ...
    def get(self, batch_size: Optional[int] = None, shuffle = True) -> Generator[RolloutBufferSamples, None, None]:
        assert self.full, ""
        if shuffle:
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def reset(self) -> None:
        ...
        for key, obs_input_shape in self.obs_shape.items():
            ...
            if key == 'graphData':
                self.observations[key] = [0 for _ in range(self.buffer_size)]
        self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.episode_starts = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(  # type: ignore[override]
        ...
        for key in self.observations.keys():
            if not isinstance(obs[key], np.ndarray):
                obs_ = Batch.from_data_list([i.to('cpu') for i in obs[key]])
            else:
                obs_ = obs[key]
            # Reshape needed when using multiple envs with discrete observations
            # as numpy cannot broadcast (n_discrete,) to (n_discrete, 1)
    ...
    def get(  # type: ignore[override]
        ...
        if shuffle:
            ...
            ordered = []
            for s in starts:
                end = min(s + block_size, total)
                ordered.extend(range(s, end))
